...
public class SecurityConfig {
	...
	public InMemoryUserDetailsManager userDetailsService() {
		...
		UserDetails admin = User.withUsername("admin").password(passwordEncoder.encode("123")).authorities("ADMIN")
				.build();
		UserDetails userLayka = User.withUsername("Layka").password(passwordEncoder.encode("123"))
		...
		return new InMemoryUserDetailsManager(admin, userLayka, user1);
	}
	...
	SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
		http.authorizeHttpRequests((requests) -> requests
				...
				.requestMatchers("/ListeCars").hasAnyAuthority("ADMIN", "AGENT", "USER").anyRequest().authenticated())
